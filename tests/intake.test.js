...
test("what agents are told about this service is true of this service", async () => {
  ...
  assert.match(guide, /`mode: full`/);
  assert.match(guide, /cooldowns control inference and CI costs and still apply after verification\s+errors/i);
...
test("a submitter is shown the problem-detection result and comments, not scores", async () => {
  ...
  // moves like that reads as a judgement it cannot carry. The page instead
  // says only whether the automated review identified problems.
  ...
  assert.match(status, /row\("Automated review"/);
  assert.match(status, /"No problems were identified\."/);
  assert.match(status, /"No blocking problems were identified\."/);
  assert.match(status, /"Problems were identified\."/);
  assert.doesNotMatch(status, /review\.passed|review\.decision/);
